...
def get_funds(address="RUbMcZS36hvfj223sRpYmchY4PEbibNcCi", rvnrpc=rvn):
    utxos = rvnrpc.getaddressutxos({"addresses": [address]})['result']
    for x in utxos:
        raw_txid = rvn.decoderawtransaction(rvn.getrawtransaction(x['txid'])['result'])['result']
        if tx_added_to_db(raw_txid):
            for y in raw_txid['vout']:
                vout = y["scriptPubKey"]
                if vout["type"] == "transfer_asset" and vout["asset"]["name"] == ASSET_NAME and vout["asset"][
                    "amount"] == 1:
